...
/*
 * "host" cpu is used by KVM and HVF, which are mutually exclusive configurations.
 */
...
void host_cpu_realizefn(DeviceState *dev, Error **errp)
{
    ...
    fprintf(stdout, "host_cpu_realizefn\n");
...
void host_cpu_initfn(Object *obj)
{
    ...
    fprintf(stdout, "host_cpu_initfn\n");
...
void host_cpu_max_initfn(X86CPU *cpu)
{
    ...
    fprintf(stdout, "host_cpu_max_initfn\n");
    
    /* initialize base class defaults */
    //max_x86_cpu_initfn(OBJECT(cpu));
...
static void host_cpu_class_init(ObjectClass *oc, void *data)
{
    ...
    fprintf(stdout, "host_cpu_class_init\n");
...
static void host_cpu_register_base_type(void)
{
    type_register_static(&host_cpu_type_info);
}

type_init(host_cpu_register_base_type);
